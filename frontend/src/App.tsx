  ...
  return (
    <div className="app-shell">
      <aside className={`sidebar ${mobileOpen ? 'sidebar-open' : ''}`}>
        ...
        <div className="condominium-switcher">
          <span>CONDOMÍNIO</span>
          <strong>Residencial Axis</strong>
          <small>Bloco A · 128 unidades</small>
        </div>
        ...
        <div className="sidebar-footer">
          <button className="nav-item" onClick={() => alert('Configurações em breve')}>
            <span className="nav-icon">⚙</span>
            Configurações
          </button>
          <div className="user-mini">
            <div className="avatar">JS</div>
            <div><strong>João Silva</strong><span>Síndico</span></div>
          </div>
        </div>
      </aside>
      ...
      <main className="main-content">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <div className="profile"><div className="avatar">JS</div><div><strong>João Silva</strong><span>Síndico</span></div></div>
          </div>
        </header>
  ...
  return (
    <div className="page">
      <div className="page-heading">
        <div><p className="eyebrow">VISÃO GERAL</p><h1>Bom dia, João 👋</h1><p className="muted">Aqui está um resumo do seu condomínio.</p></div>
        <button className="primary-button" onClick={() => onNavigate('avisos')}>+ Novo aviso</button>
      </div>

      <section className="stats-grid">
        {stats.map((stat) => <div className="stat-card" key={stat.label}><div className={`stat-icon ${stat.tone}`}>{stat.label.slice(0, 1)}</div><div><span>{stat.label}</span><strong>{stat.value}</strong><small>{stat.detail}</small></div></div>)}
      </section>

      <section className="content-grid">
        <div className="panel panel-large">
          <div className="panel-header"><div><h2>Reservas recentes</h2><p>Últimas reservas das áreas comuns</p></div><button className="text-button" onClick={() => onNavigate('reservas')}>Ver todas →</button></div>
          <div className="table-wrap"><table><thead><tr><th>Área</th><th>Solicitante</th><th>Data</th><th>Status</th></tr></thead><tbody>{reservations.map(([area, person, date]) => <tr key={area}><td><strong>{area}</strong></td><td>{person}</td><td>{date}</td><td><span className="status approved">Aprovada</span></td></tr>)}</tbody></table></div>
        </div>

        <div className="panel">
          <div className="panel-header"><div><h2>Avisos</h2><p>Comunicações recentes</p></div><button className="text-button" onClick={() => onNavigate('avisos')}>Ver todos</button></div>
          <div className="notice-list">{notices.map(([title, date, tag]) => <button className="notice-item" key={title} onClick={() => onNavigate('avisos')}><div className="notice-bullet" /><div><strong>{title}</strong><span>{date}</span></div><small>{tag}</small></button>)}</div>
        </div>
      </section>

      <section className="quick-actions">
        <div><h2>Ações rápidas</h2><p>Acesse as tarefas mais usadas pelo síndico.</p></div>
